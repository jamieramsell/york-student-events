...
class FriendshipService:
    ...
    Exposes the friend-graph use cases callers depend on -- sending, accepting,
    and removing friend requests, and querying a user's friends or whether two
    users are friends. Orchestrates the ``Friendship`` domain model from
    ``base`` with the in-memory repository, keeping persistence details out of
    callers.
    """

    def __init__(self, friendship_repository: FriendshipRepository):
        self.__friendship_repository = friendship_repository
    ...
    def send_friend_request(
        ...
        friendship = self.__friendship_repository.find_by_id(friendship_id)
        ...
        self.__friendship_repository.save(friendship)


    def accept_friend_request(self, id1: uuid.UUID, id2: uuid.UUID) -> None:
        ...
        Raises:
            ValueError: if an accepted friend request already exists between the
                two users
        ...
        if friendship is None:
            raise ValueError("No friendship record exists between the two"
                             + " users.")
    ...
    def remove_friend(self, id1: uuid.UUID, id2: uuid.UUID) -> None:
        ...
        try:
            self.__friendship_repository.delete(friendship_id)
        except KeyError:
            raise ValueError("No friendship exists between the two users.")


    def get_friends(self, user_id: uuid.UUID) -> list[uuid.UUID]:
        ...
        Returns:
            A list of the IDs of the user's friends; will never be None, but may
            be empty.
        """

        # Retrieve all friendships from the repository and initialise the user's
        # friend list.
        all_friendships = self.__friendship_repository.find_all()
        ...
        for friendship in all_friendships:
            ...
            if (user_id in friendship.get_id()
                and friendship.friendship_status
                    == base.FriendshipStatus.ACCEPTED):
    ...
    def get_pending_requests(self, user_id: uuid.UUID) -> list[uuid.UUID]:
        """Retrieves the IDs of users with a pending friendship with the user.

        This is the counterpart to ``get_friends``: it yields only friendships
        that are still awaiting acceptance, and excludes those that have been
        accepted. The direction of the request is not distinguished -- both
        requests sent by the user and requests received by the user are
        included.
        ...
        Returns:
            A list of the IDs of users with a pending request to or from the
            target user; will never be None, but may be empty.
        """
    ...
    def is_friend(self, id1: uuid.UUID, id2: uuid.UUID) -> bool:
        """Checks whether two users are friends.

        Note that the method will return True only if a friend request between
        the two users has been accepted; pending friend requests will still
        return False.
        ...
        # Predicate resolves to true if the friendship exists within the
        # repository and is accepted; returns false otherwise.
    ...
    def get_friend_circle(
        ...
        If grouping is enabled (which is the default option), results are
        generated as a dictionary, with integer keys which represent the number
        of friends between two users, and set[UUID] values, which store the IDs
        of the users at that given level.

        The users in layer 0 are those who are directly friends with the target
        user, users in layer 1 share a common friend with the user, users in
        layer 2 have a connection with the given user through two other friends,
        and so on.

        If grouping is disabled, the program uses a more efficient depth-first
        searching algorithm (as opposed to breadth-first), and returns the
        connected users' IDs in a plain set. In this mode ``max_layers`` is
        ignored: the whole reachable circle is returned, as depth-bounding a
        depth-first traversal cannot reliably yield every user within a given
        distance.
        ...
        Returns:
            If grouping is enabled, returns a ``dict[int, set[uuid.UUID]]``,
            where integer keys represent the layers (the number of friends that
            exist on the path between two users), and set values, which store
            the IDs of the users at that given level.

            If grouping is disabled, returns a simple set of connected users'
            IDs.
        ...
        # api-core has no user registry, so the friend graph is the only source
        # of truth for whether a user exists: a user is "known" if they
        # participate in at least one friendship record (pending or accepted).
        all_friendships = self.__friendship_repository.find_all()
        user_known = any(user_id in friendship.get_id()
                         for friendship in all_friendships)

        if not user_known:
            raise ValueError("The given user could not be found: either they"
                             + " don't exist, or they have no friends.")

        if grouping:
            return connections.bfs_friend_layers(
                user_id, self.get_friends, max_layers
            )

        # grouping disabled: a flat depth-first sweep of the user's whole
        # connected component -- the efficient path for large circles where only
        # reachability, not distance, matters. dfs_recursion seeds its visited
        # set with the target user, so remove them to match the grouped form
        # (which never includes the target). max_layers is intentionally not
        # threaded through here.
        reachable = connections.dfs_recursion(user_id, set(), self.get_friends)
